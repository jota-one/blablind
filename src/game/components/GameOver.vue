<template>
  <div class="flex flex-col items-center gap-8 p-6 text-center">
    ...
    <div v-if="sorted.length >= 2" class="flex items-end justify-center gap-3 w-full max-w-sm">
      ...
      <div class="flex flex-col items-center gap-2 flex-1">
        ...
        <p
          ...
          :class="sorted[1]?.id === currentPlayer.id ? 'text-primary' : ''"
        >
          {{ sorted[1]?.name }}
        </p>
        <p class="text-lg font-bold font-mono">{{ scoreLabel(sorted[1]) }}</p>
        <div
          class="w-full bg-base-content/20 rounded-t-lg h-20 flex items-center justify-center text-2xl font-bold text-base-content/40"
        >
          2
        </div>
      </div>
      ...
      <div class="flex flex-col items-center gap-2 flex-1">
        ...
        <p
          class="text-sm font-bold truncate w-full text-warning"
          :class="sorted[0]?.id === currentPlayer.id ? 'text-primary' : ''"
        >
          {{ sorted[0]?.name }}
        </p>
        <p class="text-xl font-bold font-mono text-warning">{{ scoreLabel(sorted[0]) }}</p>
        <div
          class="w-full bg-warning/30 rounded-t-lg h-32 flex items-center justify-center text-2xl font-bold text-warning/60"
        >
          1
        </div>
      </div>
      ...
      <div v-if="sorted[2]" class="flex flex-col items-center gap-2 flex-1">
        ...
        <p
          class="text-sm font-semibold truncate w-full"
          :class="sorted[2]?.id === currentPlayer.id ? 'text-primary' : ''"
        >
          {{ sorted[2]?.name }}
        </p>
        <p class="text-lg font-bold font-mono">{{ scoreLabel(sorted[2]) }}</p>
        <div
          class="w-full bg-base-content/10 rounded-t-lg h-16 flex items-center justify-center text-2xl font-bold text-base-content/30"
        >
          3
        </div>
      </div>
    ...
    <div v-if="sorted.length > 3" class="w-full max-w-sm space-y-1">
      <p class="text-xs text-base-content/40 uppercase tracking-wide mb-2">
        {{ t('gameover.ranking_rest') }}
      </p>
      <div
        ...
        :class="[
          ...
          p.id === currentPlayer.id ? 'bg-primary/10 border border-primary/30' : 'bg-base-200',
        ]"
      >
    ...
    <div v-if="doneTracks.length > 0" class="w-full max-w-sm space-y-1 text-left">
      ...
      >
        <div class="flex-1 min-w-0">
          ...
          <p v-if="track.expand?.video?.artist" class="text-xs text-base-content/50 truncate">
