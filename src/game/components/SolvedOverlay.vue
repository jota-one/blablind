<template>
  <div
    :class="[
      'solved-overlay fixed inset-0 z-50 flex items-center justify-center',
      isWinner || type === 'buzzed' ? 'bg-success/85' : 'bg-black/70',
    ]"
  >
    <div
      :class="[
        'solved-card rounded-2xl p-8 flex flex-col items-center gap-3 shadow-2xl text-center max-w-xs mx-4',
        isWinner || type === 'buzzed' ? 'bg-success text-success-content' : 'bg-base-100',
      ]"
    >
      ...
      <template v-else>
        ...
        <p class="text-xl font-bold font-display mt-1">
          {{ t('overlay.solved', { player: playerName }) }}
        </p>
      </template>
      <div v-if="title || artist" class="space-y-0.5 mt-1">
        ...
        <p
          ...
          :class="[
            'text-base',
            isWinner || type === 'buzzed' ? 'text-success-content/70' : 'text-base-content/60',
          ]"
        >
          {{ artist }}
        </p>
