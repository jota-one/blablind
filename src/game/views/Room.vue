<template>
  <div class="h-screen flex flex-col overflow-hidden">
    ...
    <header class="shrink-0 bg-base-100/90 backdrop-blur border-b border-base-300">
      ...
      <div class="px-4 py-3 flex items-center gap-3">
        ...
        <button
          ...
          :class="[
            ...
            canTogglePause ? 'cursor-pointer' : 'cursor-default',
          ]"
          :title="isPaused ? t('room.resume') : t('room.pause')"
          @click="togglePause"
        >
          {{ isPaused ? t('room.status_paused') : sessionStatusLabel }}
        </button>
        <span
          v-else
          :class="[
        ...
        <button
          type="button"
          class="btn btn-sm btn-ghost relative shrink-0"
          :title="t('room.menu')"
          @click="showMenuDrawer = true"
        >
          <span class="i-fa6-solid-bars text-base"></span>
          <span
            v-if="hasPendingRoleRequest"
            class="absolute top-1 right-1 h-2 w-2 rounded-full bg-error"
          ></span>
        </button>
      </div>
      ...
      >
        ...
        <span class="flex items-center gap-1 shrink-0"
          ><span class="i-fa-solid-users text-[0.65rem]"></span>{{ onlinePlayers.length }}</span
        >
      ...
      <div v-else class="flex flex-col gap-4 p-4 min-w-0">
        ...
        <div
          :class="[
            ...
            videoId
              ? 'relative ' + (isCurrentTrackAdmin ? 'aspect-video lg:max-h-56' : 'h-20')
              : '',
          ]"
        >
          ...
          <div
            ...
            :class="{ 'opacity-0 pointer-events-none': audioUnlocked || !videoId }"
          >
            <YoutubePlayer
              :video-id="videoId"
          ...
          >
            <template v-if="currentTrack">
              <div
                :class="[
                  'transition-all',
                  activeBuzz && !isAutonomousMode
                    ? 'opacity-50'
                    : isCurrentTrackAdmin
                      ? 'animate-bounce'
                      : '',
                  isCurrentTrackAdmin ? 'text-7xl' : 'text-3xl shrink-0',
                ]"
              >
                🎵
              </div>
              <div :class="isCurrentTrackAdmin ? 'text-center' : 'min-w-0'">
                <p
                  :class="[
                    ...
                    isCurrentTrackAdmin ? 'text-xl px-4' : 'text-sm truncate',
                  ]"
                >
                  <template v-if="isCurrentTrackAdmin">
                    {{ currentTrack.expand?.video?.title || t('room.no_title') }}
                    <span
                      v-if="currentTrack.expand?.video?.artist"
                      class="block text-base font-normal text-base-content/60"
                      >{{ currentTrack.expand?.video?.artist }}</span
                    >
                  </template>
                  <template v-else-if="isAutonomousMode">
                    <span class="text-base-content/40">{{
                      t('room.auto_track_counter', autoTrackCounter)
                    }}</span>
                  </template>
                  <template v-else>
                    <span class="text-base-content/40">{{
                      t('room.track_by', { player: getPlayerName(currentTrack.added_by) })
                    }}</span>
                  </template>
                </p>
                <p
                  v-if="activeBuzz && !isAutonomousMode"
                  :class="[
                    'text-warning font-semibold animate-pulse',
                    isCurrentTrackAdmin ? 'text-sm' : 'text-xs',
                  ]"
                >
                  {{ t('room.in_pause') }}
                </p>
                <template v-else>
                  <p
                    v-if="isTrackSolvedAndPlaying"
                    :class="[
                      'text-success font-semibold',
                      isCurrentTrackAdmin ? 'text-xs' : 'text-xs',
                    ]"
                  >
                  ...
                  <p
                    v-else
                    :class="['text-base-content/40', isCurrentTrackAdmin ? 'text-xs' : 'text-xs']"
                  >
                    {{ t('room.playing') }}
                  </p>
            ...
            <template v-else>
              ...
              <template v-if="session.status === 'waiting'">
                ...
                <div class="w-full max-w-xs space-y-2">
                  <p
                    class="text-xs uppercase tracking-wide font-semibold text-base-content/40 px-1"
                  >
                    {{ t('room.participants') }}
                  </p>
                  <ul class="space-y-1.5">
                    ...
                    >
                      ...
                      <span
                        v-else
                        class="w-8 h-8 shrink-0 rounded-full bg-primary/15 text-primary text-sm font-bold flex items-center justify-center"
                        >{{ initial(p.name) }}</span
                      >
                      <span class="flex-1 min-w-0 truncate font-medium text-sm">
                        {{ p.name }}
                        <span v-if="p.id === currentPlayer.id" class="text-xs text-base-content/40"
                          >({{ t('room.you') }})</span
                        >
                      </span>
                      <span v-if="p.id === session.host" class="badge badge-warning badge-sm gap-1"
                        >👑 {{ t('room.host_badge') }}</span
                      >
                      <span v-else-if="p.ready" class="badge badge-success badge-sm gap-1"
                        ><span class="i-fa-solid-check text-[0.6rem]"></span
                        >{{ t('room.ready_badge') }}</span
                      >
                      <span v-else class="badge badge-ghost badge-sm text-base-content/50">{{
                        t('room.player_waiting')
                      }}</span>
                    </li>
                  </ul>
                </div>
                ...
                <template v-if="isHost">
                  <button
                    class="btn btn-primary btn-lg max-w-full"
                    :disabled="!canLaunch"
                    @click="launchSession"
                  >
                ...
                <template v-else>
                  ...
                  <div v-else class="flex flex-col items-center gap-2">
                    ...
                    <button class="btn btn-xs btn-ghost" @click="markReady(false)">
                      {{ t('room.ready_cancel') }}
                    </button>
                  </div>
                </template>
              ...
              <template v-else>
                ...
                <template v-if="isHost">
                  <button
                    ...
                    @click="startTrack(queuedTracks[0])"
                  >
                    <span class="i-fa-solid-play"></span>
                    {{ t('room.play_next') }}
                  </button>
        ...
        <div
          v-if="
            !isAutonomousMode && currentTrack && !isCurrentTrackAdmin && !isTrackSolvedAndPlaying
          "
          class="w-full"
        >
          <div v-if="activeBuzz && activeBuzz.player === currentPlayer.id" class="alert alert-info">
            ...
            <div>
              <p class="font-bold">
                {{ isIrlMode ? t('room.buzz_irl') : t('room.buzz_submitted') }}
              </p>
          ...
          <div v-else-if="activeBuzz" class="alert">
            ...
            <span>{{
              t('room.buzz_answering', { player: getPlayerName(activeBuzz.player) })
            }}</span>
          </div>
          <template v-else>
            <div v-if="buzzing && !isIrlMode" class="card bg-base-200 p-4 space-y-3">
              ...
              <div class="flex gap-2">
                ...
                <button class="btn btn-ghost" @click="buzzing = false">
                  {{ t('room.buzz_cancel') }}
                </button>
              </div>
            </div>
            ...
            <div v-else class="alert alert-warning alert-soft">
              ...
              <template v-if="buzzBlockReason === 'max_attempts'">{{
                t('room.buzz_wait_max')
              }}</template>
              <template v-else-if="buzzBlockReason === 'delay'">{{
                t('room.buzz_wait_delay', { s: rebuzzRemainingSeconds })
              }}</template>
        ...
        <div
          v-if="currentTrack && isCurrentTrackAdmin && activeBuzz"
        ...
        >
          ...
          <p
            v-if="sessionSettings.auto_reject_delay > 0 && autoRejectRemainingSeconds > 0"
            class="text-xs text-base-content/40 text-center tabular-nums"
          >
          ...
          <div class="flex gap-2">
            ...
            <button
              ...
              data-testid="validate-wrong"
              @click="invalidateBuzz"
            >
        ...
        <div
          v-if="
            ...
            !isTrackSolvedAndPlaying
          "
          class="text-xs text-base-content/50 text-center"
        >
          <span class="i-fa-solid-forward-step text-xs mr-1"></span
          >{{ t('room.skip_voters', { names: skipVoterNames.join(', ') }) }}
        </div>

        <!-- Orphan tracks notification (host only) -->
        <div
          v-if="isHost && nextOrphanOwner && !orphanDecisionTrack"
          class="card bg-warning/10 border border-warning/30 p-4 space-y-3"
        >
          <p class="font-bold flex items-center gap-2 text-sm">
            <span class="i-fa-solid-user-slash text-warning shrink-0"></span>
            {{
              t('room.orphan_notification', {
                player: nextOrphanOwner.name,
                n: orphanedQueuedTracks.length,
              })
            }}
          </p>
        ...
        <div>
          <div class="flex items-end gap-0.5 border-b border-base-300">
            <button
              :class="[
                'flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-t-lg border select-none transition-colors',
                activeTab === 'upcoming'
            ...
            >
              {{ t('room.tab_upcoming') }}
              <span v-if="upcomingTracks.length" class="badge badge-xs">{{
            ...
            >
              {{ t('room.tab_done') }}
              <span v-if="doneTracks.length" class="badge badge-xs" data-testid="done-count">{{
                doneTracks.length
              }}</span>
            </button>
            <button
          ...
          <div class="border border-base-300 border-t-0 rounded-b-xl bg-base-100 px-3 pb-3">
            <div ref="tabs-outer" class="overflow-hidden touch-pan-y">
              ...
              <div
                v-if="isHost && hostCandidate"
                class="alert alert-info mt-3 flex items-center justify-between gap-2"
              >
                <span class="text-sm">{{
                  t('room.host_candidate_banner', { name: hostCandidate.name })
                }}</span>
                <div class="flex gap-2 shrink-0">
                  <button class="btn btn-xs btn-success" @click="approveHost">
                    {{ t('room.host_accept') }}
                  </button>
                  <button class="btn btn-xs btn-ghost" @click="rejectHost">
                    {{ t('room.host_reject') }}
                  </button>
                </div>
              </div>
              ...
              <div
                v-if="isIrlMode && isDJ && djCandidate"
                class="alert alert-info mt-3 flex items-center justify-between gap-2"
              >
                <span class="text-sm">{{
                  t('room.dj_candidate_banner', { name: djCandidate.name })
                }}</span>
                <div class="flex gap-2 shrink-0">
                  <button class="btn btn-xs btn-success" @click="approveDJ">
                    {{ t('room.dj_accept') }}
                  </button>
                  <button class="btn btn-xs btn-ghost" @click="rejectDJ">
                    {{ t('room.dj_reject') }}
                  </button>
                </div>
              </div>
              ...
              >
                ...
                <div class="w-full shrink-0 pt-3 space-y-3">
                  <div
                    v-if="
                      myQueuedTracks.length >= 2 ||
                      (isHost && queuedTracks.length >= 2 && (!currentTrack || isPaused))
                    "
                    class="flex justify-end gap-2"
                  >
                    <button
                      v-if="isHost && queuedTracks.length >= 2 && (!currentTrack || isPaused)"
                    ...
                    <button
                      v-if="myQueuedTracks.length >= 2"
                      class="btn btn-xs btn-ghost text-base-content/50"
                      @click="shuffleMyTracks"
                    >
                      <span class="i-fa6-solid-shuffle"></span>
                  ...
                  <ul v-if="doneTracks.length > 0" class="space-y-1">
                    ...
                    >
                      ...
                      <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium truncate">
                          {{ track.expand?.video?.title || t('room.no_title') }}
                          <span v-if="isMyTrack(track)" class="badge badge-xs badge-primary ml-1">{{
                            t('room.my_badge')
                          }}</span>
                        </p>
                        <p v-if="track.expand?.video?.artist" class="text-xs text-base-content/50">
                          {{ track.expand?.video?.artist }}
                        </p>
                      ...
                      <FavoriteButton
                        :active="isFavorite(track)"
                        @toggle="onFavoriteToggle(track)"
                      />
                    </li>
                  </ul>
                  <p v-else class="text-sm text-center text-base-content/40 py-4">
                    ...
                    >
                      <span
                        :class="[
                          ...
                          i === 0 ? 'text-warning' : 'text-base-content/40',
                        ]"
                        >{{ i + 1 }}</span
                      >
                      <img
                      ...
                      <span
                        v-else
                        class="w-7 h-7 shrink-0 rounded-full bg-primary/15 text-primary text-xs font-bold flex items-center justify-center"
                        :class="!isOnline(p) ? 'opacity-40' : ''"
                        >{{ initial(p.name) }}</span
                      >
                      <span
                      ...
                      <span
                        v-if="isIrlMode && p.id === session.dj_player"
                        title="DJ"
                        class="text-base"
                        >🎵</span
                      >
                      ...
                      >
                        <span class="font-mono font-bold text-primary">
                          {{
                            playerRatio(p).guessable === 0
                              ? '—'
                              : `${parseFloat((playerRatio(p).ratio * 100).toFixed(2))}%`
                          }}
                        </span>
                        <span
                          v-if="playerRatio(p).guessable > 0"
                      ...
                      <span
                        v-if="!isOnline(p)"
                        class="w-2 h-2 rounded-full bg-base-content/20 shrink-0"
                        :title="t('room.offline')"
                      ></span>
                      <span
                        v-else-if="activeBuzz?.player === p.id"
                        class="i-fa-solid-bell text-warning animate-bounce text-xs"
                      ></span>
                    </li>
                  </ul>
                  <p
    ...
    <div
      v-if="
        awaitingAdvance &&
        !animationState &&
        (sessionSettings.stop_method === 'vote_unanimous' ||
          (sessionSettings.stop_method === 'host_choice' && isHost))
      "
      class="fixed inset-0 z-50 flex items-center justify-center bg-black/70"
    >
      <div
        class="bg-base-100 rounded-2xl p-8 flex flex-col items-center gap-4 shadow-2xl text-center max-w-xs mx-4"
      >
        ...
        <div class="space-y-1">
          <p class="text-lg font-bold font-display">
            ...
            <FavoriteButton
              :active="isFavorite(currentTrack)"
              @toggle="onFavoriteToggle(currentTrack)"
            />
          </p>
        ...
        <template v-else>
          ...
          <p class="text-xs text-base-content/40">
            {{ t('room.still_playing_votes', { votes: skipVoteCount, needed: skipVotesNeeded }) }}
          </p>
        </template>
      </div>
    </div>
    ...
    <div :class="['modal', isHost && orphanDecisionTrack ? 'modal-open' : '']">
      <div class="modal-box max-w-sm">
        ...
        <p class="text-sm font-medium truncate mb-3">
          🎵 {{ orphanDecisionTrack?.expand?.video?.title ?? t('room.no_title') }}
        </p>
      ...
      <div class="modal-box max-w-sm">
        ...
        <div class="space-y-2">
          ...
          <div class="flex items-center gap-3">
            ...
            <span
              v-else
              class="w-9 h-9 shrink-0 rounded-full bg-primary/15 text-primary text-sm font-bold flex items-center justify-center"
              >{{ initial(hostPlayer?.name ?? '?') }}</span
            >
            <span class="flex-1 min-w-0 truncate font-medium"
              >👑 {{ hostPlayer?.name ?? '—' }}</span
            >
          </div>
          <!-- Pending request (host sees accept/reject) -->
          <div
            v-if="isHost && hostCandidate"
            class="alert alert-info py-2 text-sm flex items-center justify-between gap-2"
          >
            <span>{{ t('room.host_candidate_banner', { name: hostCandidate.name }) }}</span>
            <div class="flex gap-2 shrink-0">
              <button class="btn btn-xs btn-success" @click="approveHostFromRoles">
                {{ t('room.host_accept') }}
              </button>
              <button class="btn btn-xs btn-ghost" @click="rejectHost">
                {{ t('room.host_reject') }}
          ...
          <p
            v-else-if="!isHost && session.host_candidate === currentPlayer.id"
            class="text-sm text-base-content/50 text-center"
          >
            {{ t('room.host_pending') }}
          </p>
        </div>
        ...
        <template v-if="isIrlMode">
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-3">
              ...
              <span
                v-else
                class="w-9 h-9 shrink-0 rounded-full bg-accent/15 text-accent text-sm font-bold flex items-center justify-center"
                >{{ initial(djPlayer?.name ?? '?') }}</span
              >
              <span class="flex-1 min-w-0 truncate font-medium"
                >🎵 {{ djPlayer?.name ?? '—' }}</span
              >
            ...
            <div
              v-if="isDJ && djCandidate"
              class="alert alert-info py-2 text-sm flex items-center justify-between gap-2"
            >
              <span>{{ t('room.dj_candidate_banner', { name: djCandidate.name }) }}</span>
              <div class="flex gap-2 shrink-0">
                <button class="btn btn-xs btn-success" @click="approveDJFromRoles">
                  {{ t('room.dj_accept') }}
                </button>
                <button class="btn btn-xs btn-ghost" @click="rejectDJ">
                  {{ t('room.dj_reject') }}
            ...
            <p
              v-else-if="!isDJ && session.dj_candidate === currentPlayer.id"
              class="text-sm text-base-content/50 text-center"
            >
              {{ t('room.dj_pending') }}
            </p>
        ...
        <div class="modal-action">
          <button class="btn btn-ghost btn-sm" @click="showRolesModal = false">
            {{ t('room.reset_cancel') }}
          </button>
        </div>
      </div>
    ...
    <div :class="['modal', showSettingsModal ? 'modal-open' : '']">
      <div class="modal-box max-w-sm">
        ...
        <div class="space-y-3 text-sm">
          ...
          <div v-if="isAutonomousMode" class="flex items-center justify-between gap-4">
            ...
            <div v-if="isHost" class="flex items-center gap-1">
              <input
                v-model.number="editedSettings.default_playback_duration"
                type="number"
                min="5"
                max="120"
            ...
            <span v-else class="font-mono font-bold"
              >{{ sessionSettings.default_playback_duration }}s</span
            >
          </div>
          ...
          <template v-if="!isAutonomousMode">
            <div class="flex items-center justify-between gap-4">
              ...
              <input
                v-if="isHost"
                v-model.number="editedSettings.max_buzz_attempts"
                type="number"
                min="1"
                max="20"
                class="input input-xs w-16 text-right"
              />
              <span v-else class="font-mono font-bold">{{
                sessionSettings.max_buzz_attempts
              }}</span>
            </div>
            ...
            <div class="flex items-center justify-between gap-4">
              ...
              <div v-if="isHost" class="flex items-center gap-1">
                <input
                  v-model.number="editedSettings.rebuzz_delay"
                  type="number"
                  min="0"
                  max="60"
                  class="input input-xs w-16 text-right"
            ...
            <div class="flex items-center justify-between gap-4">
              ...
              <div v-if="isHost" class="flex items-center gap-1">
                <input
                  v-model.number="editedSettings.auto_reject_delay"
                  type="number"
                  min="0"
                  max="60"
                  class="input input-xs w-16 text-right"
              ...
              <span v-else class="font-mono font-bold"
                >{{ sessionSettings.auto_reject_delay }}s</span
              >
            </div>
            ...
            <div class="flex items-center justify-between gap-4">
              ...
              <input
                v-if="isHost"
                v-model="editedSettings.continue_after_success"
                type="checkbox"
                class="toggle toggle-sm toggle-primary"
              />
              <span v-else class="font-mono font-bold">{{
                sessionSettings.continue_after_success ? '✓' : '✗'
              }}</span>
            </div>
            ...
            <div
              v-if="
                isHost
                  ? editedSettings.continue_after_success
                  : sessionSettings.continue_after_success
              "
            ...
            <div class="flex items-center justify-between gap-4">
              ...
              <input
                v-if="isHost"
                v-model="editedSettings.force_equity"
                type="checkbox"
                class="toggle toggle-sm toggle-primary"
              />
              <span v-else class="font-mono font-bold">{{
                sessionSettings.force_equity ? '✓' : '✗'
              }}</span>
            </div>

            <div
              v-if="isHost ? editedSettings.force_equity : sessionSettings.force_equity"
              class="flex items-center justify-between gap-4"
            >
              <span class="text-base-content/70">{{
                t('admin.settings_equity_margin_label')
              }}</span>
              <div v-if="isHost" class="flex items-center gap-1">
                <input
                  v-model.number="editedSettings.equity_margin"
                  type="number"
                  min="1"
                  max="10"
                  class="input input-xs w-16 text-right"
        ...
        <div class="modal-action">
          <button class="btn btn-ghost btn-sm" @click="showSettingsModal = false">
            {{ t('room.reset_cancel') }}
          </button>
          <button
            v-if="isHost"
    ...
    <div :class="['modal', showResetModal ? 'modal-open' : '']">
      <div class="modal-box">
        ...
        <div class="modal-action">
          <button class="btn btn-ghost" :disabled="resetting" @click="showResetModal = false">
            {{ t('room.reset_cancel') }}
          </button>
    ...
    <div v-show="showAddTrackModal" class="fixed inset-0 z-50 bg-base-100 flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div class="tabs tabs-bordered">
          ...
          <button
            :class="['tab', addMode === 'single' ? 'tab-active' : '']"
        ...
        <template v-if="addMode === 'single'">
          <input
            ...
            type="url"
            :placeholder="t('room.url_placeholder')"
            class="input input-bordered w-full"
          />
          <div class="flex flex-col gap-2">
            <div class="flex-1">
              <input
                ...
                type="number"
                :placeholder="t('room.start_placeholder')"
                class="input input-bordered w-full"
                min="0"
              />
            </div>
            <div class="flex gap-2">
              <input
                v-model.number="newTrack.playback_duration"
                type="number"
                :placeholder="t('track.playback_duration_title')"
                class="input input-bordered flex-1"
                min="1"
                :title="t('track.playback_duration_title')"
              />
              <input
                v-model.number="newTrack.reveal_seconds"
                type="number"
                :placeholder="t('track.reveal_seconds_title')"
                class="input input-bordered flex-1"
                min="0"
            ...
            <div class="flex-1 relative">
              ...
              <span
                v-if="fetchingMeta"
                class="loading loading-spinner loading-xs absolute right-3 top-1/2 -translate-y-1/2 text-base-content/30"
              ></span>
            </div>
            <div class="flex-1 relative">
              <input
                ...
                type="text"
                :placeholder="t('room.artist_placeholder')"
                class="input input-bordered w-full"
              />
              <span
                v-if="fetchingMeta"
          ...
          <button
            class="btn btn-primary w-full"
            :disabled="!newTrack.youtube_url.trim() || addingTrack || !canAddTrack"
  ...
  <div v-if="myDuplicateTrack" class="modal modal-open">
    <div class="modal-box">
      ...
      <p class="py-3 text-sm">
        {{
          t('room.duplicate_body', {
            title: myDuplicateTrack.expand?.video?.title ?? t('room.no_title'),
          })
        }}
      </p>
      <div class="modal-action">
        <button class="btn btn-ghost" @click="dismissDuplicate(true)">
          {{ t('room.duplicate_keep') }}
        </button>
        <button class="btn btn-error" @click="dismissDuplicate(false)">
          {{ t('room.duplicate_delete') }}
        </button>
      </div>
    </div>
  </div>
  ...
  <Transition name="drawer-slide">
    <aside
      v-if="showMenuDrawer"
      class="fixed top-0 right-0 bottom-0 z-50 w-72 max-w-[85vw] bg-base-100 shadow-xl flex flex-col"
    >
      ...
      <nav class="flex-1 overflow-y-auto p-2 flex flex-col gap-1">
        ...
        <template v-if="isHost">
          ...
          <label class="btn btn-ghost justify-start gap-3 cursor-pointer">
            <span
              class="i-fa6-solid-people-group text-base"
              :class="isIrlMode ? 'text-accent' : 'text-base-content/60'"
            ></span>
        ...
        <button
          v-if="canClaim"
          class="btn btn-ghost justify-start gap-3 text-primary"
          @click="claimSessionFromMenu"
        >
  ...
  <div :class="['modal', showParticipantsModal ? 'modal-open' : '']">
    <div class="modal-box max-w-sm">
      ...
      <div class="modal-action">
        <button class="btn btn-ghost btn-sm" @click="showParticipantsModal = false">
          {{ t('room.reset_cancel') }}
        </button>
      </div>
    </div>
...
const requestSeek = (seconds: number) => {
  seekRequest.value = { seconds, token: ++seekToken }
}
const {
  tracks,
  currentTrack,
...
const otherEligibleCount = computed(
  () =>
    onlinePlayers.value.filter(
      p => p.id !== props.currentPlayer.id && p.id !== trackValidatorId.value,
    ).length,
)
...
const {
  votes: answerVotes,
  myVoteForBuzz,
  castVote: castAnswerVote,
  reload: reloadVotes,
} = useAnswerVotes(computed(() => (isAutonomousMode.value ? currentTrack.value?.id : undefined)))
...
const clearPlaybackTimer = () => {
  if (playbackDurationTimer) {
    clearTimeout(playbackDurationTimer)
    playbackDurationTimer = null
  }
}
...
watch(
  () => isAutonomousMode.value && autoPhase.value === 'voting',
  voting => {
    if (!voting) return
    clearPlaybackTimer()
    pausedByDuration.value = false
...
watch(
  () => isAutonomousMode.value && autoPhase.value === 'guessing' && !pausedByDuration.value,
  active => {
    if (active && !windowClock) {
      windowNow.value = Date.now()
      windowClock = setInterval(() => {
        windowNow.value = Date.now()
      }, 250)
...
const canClaim = computed(
  () => isHost.value && isAuthenticated.value && user.value?.id && !props.session.owner,
)

// The original session creator (authenticated owner) can reclaim host instantly.
const isOwner = computed(
  () => isAuthenticated.value && !!user.value?.id && props.session.owner === user.value.id,
)
const hostCandidate = computed(() =>
  players.value.find((p: any) => p.id === props.session.host_candidate),
)
...
const isReady = computed(
  () => players.value.find(p => p.id === props.currentPlayer.id)?.ready ?? false,
)
const canLaunch = computed(
  () =>
    allNonHostPlayersReady.value &&
    queuedTracks.value.length > 0 &&
    onlinePlayers.value.length >= 2,
)
...
const skipVotesNeeded = computed(() =>
  computeSkipVotesNeeded({
    ...
    onlineNonValidatorCount: onlinePlayers.value.filter(p => p.id !== trackValidatorId.value)
      .length,
  }),
)
const hasVotedToSkip = computed(() => skipVoteArray.value.includes(props.currentPlayer.id))
const skipVoterNames = computed(() =>
  skipVoteArray.value.filter(id => id !== trackValidatorId.value).map(id => getPlayerName(id)),
)
const isTrackSolvedAndPlaying = computed(
  () =>
    (!!currentTrack.value?.solved_by || !!currentTrack.value?.skip_revealed) &&
    currentTrack.value?.status === 'playing',
)
...
const orphanInherit = async () => {
  await Promise.all(
    orphanedQueuedTracks.value.map(t =>
...
const orphanSplit = async () => {
  ...
  await Promise.all(
    orphanedQueuedTracks.value.map((t, i) =>
...
const batchUpdateOrders = async (updates: { id: string; order: number }[]) => {
  for (let i = 0; i < updates.length; i += 45) {
    ...
    updates
      .slice(i, i + 45)
      .forEach(({ id, order }) => batch.collection('tracks').update(id, { order }))
    await batch.send()
  }
}
...
const getPlayerName = (playerId: string) =>
  players.value.find(p => p.id === playerId)?.name ?? t('room.unknown_player')
...
const autoRejectRemainingSeconds = computed(() => {
  if (
    !activeBuzz.value ||
    !isCurrentTrackAdmin.value ||
    sessionSettings.value.auto_reject_delay <= 0
  )
    return 0
  const deadline =
    new Date(activeBuzz.value.created).getTime() + sessionSettings.value.auto_reject_delay * 1000
  return Math.max(0, Math.ceil((deadline - autoRejectNow.value) / 1000))
})

watch(
  [activeBuzz, isCurrentTrackAdmin],
  ([buzz, isAdmin]) => {
    if (autoRejectTimer) {
    ...
    if (autoRejectClock) {
      clearInterval(autoRejectClock)
      autoRejectClock = null
    }
    if (
      buzz &&
      (buzz as any).status === 'pending' &&
      isAdmin &&
      sessionSettings.value.auto_reject_delay > 0
    ) {
      autoRejectClock = setInterval(() => {
        autoRejectNow.value = Date.now()
      }, 500)
...
const approveDJ = () =>
  pb
    .collection('sessions')
    .update(props.session.id, { dj_player: props.session.dj_candidate, dj_candidate: null })
const rejectDJ = () => pb.collection('sessions').update(props.session.id, { dj_candidate: null })
...
const proposeHost = () => {
  if (isOwner.value) {
    return pb
      .collection('sessions')
      .update(props.session.id, { host: props.currentPlayer.id, host_candidate: null })
  }
  return pb
    .collection('sessions')
    .update(props.session.id, { host_candidate: props.currentPlayer.id })
}
const approveHost = () =>
  pb
    .collection('sessions')
    .update(props.session.id, { host: props.session.host_candidate, host_candidate: null })
const rejectHost = () =>
...
const resetSession = async () => {
  ...
  try {
    ...
    const allOps: Array<(b: ReturnType<typeof pb.createBatch>) => void> = [
      ...allBuzzes.map(
        buzz => (b: ReturnType<typeof pb.createBatch>) => b.collection('buzzes').delete(buzz.id),
      ),
...
const saveSettings = async () => {
  ...
  try {
    await pb
      .collection('sessions')
      .update(props.session.id, { settings: { ...editedSettings.value } })
...
const endSession = () => pb.collection('sessions').update(props.session.id, { status: 'finished' })
