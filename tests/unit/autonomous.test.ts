...
check('guessing, window end, no buzz', computeNextAction(base({ windowElapsed: true })), {
  type: 'no-winner-reveal',
})
check(
  'guessing, window end, buzz, remote',
  computeNextAction(
    base({
      windowElapsed: true,
...
check(
  'guessing, all online buzzed (early close)',
  computeNextAction(
    base({
      buzzes: [
...
check(
  'voting, 1 yes of 2 voters → correct',
  computeNextAction({
    ...votingBase,
...
check(
  'voting, 1 no → still open (yes reachable)',
  computeNextAction({
    ...votingBase,
...
check(
  'voting, candidate own vote ignored',
  computeNextAction({
    ...votingBase,
...
check(
  'voting, next candidate after rejection',
  computeNextAction({
    ...votingBase,
    buzzes: [
...
check(
  '6p, 3 no → wrong (yes unreachable: max 2 < 3)',
  computeNextAction({
    ...six,
    votes: [
      { buzz: 'b1', voter: 'b', value: false },
...
// Auto-end
check(
  'between tracks, queue left → idle',
  computeNextAction(base({ hasCurrentTrack: false, queuedCount: 1, doneCount: 1 })),
  null,
)
check(
  'queue empty, done > 0 → end',
  computeNextAction(base({ hasCurrentTrack: false, queuedCount: 0, doneCount: 3 })),
  { type: 'end-session' },
)
check(
  'waiting session → idle',
  computeNextAction(
    base({ sessionStatus: 'waiting', hasCurrentTrack: false, queuedCount: 0, doneCount: 3 }),
  ),
  null,
)
