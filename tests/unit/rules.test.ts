...
const settings = (max_buzz_attempts: number, rebuzz_delay: number) => ({
  max_buzz_attempts,
  rebuzz_delay,
})
...
check(
  'pending buzz blocks (returns null, canBuzz gate handles it)',
  buzzBlockReason(
    [wrongBuzz('me', T0), { player: 'x', status: 'pending', created: T1, updated: T1 }],
    'me',
    settings(5, 5),
    at(T1),
    2,
...
check(
  'empty player id → null',
  buzzBlockReason([wrongBuzz('me', T0)], '', settings(5, 5), at(T1), 2),
  null,
)
check(
  'max attempts reached',
  buzzBlockReason([wrongBuzz('me', T0)], 'me', settings(1, 5), at(T1), 2),
  'max_attempts',
)
check(
  'within rebuzz delay',
  buzzBlockReason([wrongBuzz('me', T0)], 'me', settings(5, 10), at(T1), 2),
  'delay',
)
check(
  'delay elapsed → free',
  buzzBlockReason([wrongBuzz('me', T0)], 'me', settings(5, 3), at(T1), 2),
  null,
)
check(
  'delay counts from updated, not created',
  buzzBlockReason([wrongBuzz('me', T0, T1)], 'me', settings(5, 3), at(T1) + 1000, 2),
  'delay',
)
check(
  'no delay: blocked until another player buzzes',
  buzzBlockReason([wrongBuzz('me', T0)], 'me', settings(5, 0), at(T1), 2),
  'others',
)
check(
  'no delay: another player buzzed since → free',
  buzzBlockReason([wrongBuzz('me', T0), wrongBuzz('other', T1)], 'me', settings(5, 0), at(T1), 2),
  null,
)
check(
  'no delay, no other eligible player → free',
  buzzBlockReason([wrongBuzz('me', T0)], 'me', settings(5, 0), at(T1), 0),
  null,
)
...
check(
  ...
  skipVotesNeeded({
    hasTrack: false,
    solvedAndPlaying: false,
    onlineCount: 4,
    onlineNonValidatorCount: 3,
  }),
  1,
)
check(
  'unsolved → online minus validator',
  skipVotesNeeded({
    hasTrack: true,
    solvedAndPlaying: false,
    onlineCount: 4,
...
check(
  'solved & playing → everyone online',
  skipVotesNeeded({
    hasTrack: true,
    solvedAndPlaying: true,
    onlineCount: 4,
    onlineNonValidatorCount: 3,
  }),
  4,
...
check(
  'floor 1 (nobody else online)',
  skipVotesNeeded({
    hasTrack: true,
    solvedAndPlaying: false,
    onlineCount: 1,
    onlineNonValidatorCount: 0,
  }),
  1,
)
...
check('own tracks excluded from guessable', playerRatio(done, 'b'), {
  guessed: 1,
  guessable: 2,
  ratio: 0.5,
})
check('two guessed out of three', playerRatio(done, 'a'), {
  guessed: 2,
  guessable: 3,
  ratio: 2 / 3,
})
check('no guessable → ratio 0', playerRatio([{ added_by: 'a', solved_by: '' }], 'a'), {
  guessed: 0,
  guessable: 0,
  ratio: 0,
})
...
check(
  'tie-break sanity',
  rankPlayers([{ id: 'a' }, { id: 'b' }], tieDone).map(p => p.id),
  ['b', 'a'],
)
